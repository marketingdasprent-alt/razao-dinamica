...
for (const oldEvents of [false, true]) test(`Permissões com migração antiga de eventos: ${oldEvents}`, async t => {
  ...
  await t.test('Cada sessão só regista o próprio tipo de dispositivo', async () => {
    await as(null)
    await assert.rejects(db.query("select public.registar_dispositivo('mobile')"))
    await as(2)
    await assert.rejects(db.query("select public.registar_dispositivo('tablet')"))
    await db.query("select public.registar_dispositivo('mobile')")
    assert.equal((await rows('select ultimo_dispositivo from perfis where id=$1', [ids[2]]))[0].ultimo_dispositivo, 'mobile')
