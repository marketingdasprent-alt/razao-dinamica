...
for (const oldEvents of [false, true]) test(`Permissões com migração antiga de eventos: ${oldEvents}`, async t => {
  ...
  await t.test('Exclusão de utilizador: protege o último admin e recusa enquanto houver leads', async () => {
    await as(0)
    await assert.rejects(db.query('select public.excluir_perfil($1)', [ids[0]]))
    await db.query("insert into leads(nome,email,estado) values ('Throwaway','throwaway@test.invalid','Novo')")
    const [novo] = await rows("select * from leads where email='throwaway@test.invalid'")
    await db.query('select public.atribuir_lead($1,$2,$3)', [novo.id, ids[2], novo.atualizado_em])
    await assert.rejects(db.query('select public.excluir_perfil($1)', [ids[2]]))
    assert.equal((await rows('select * from perfis where id=$1', [ids[2]])).length, 1)
    await db.query('delete from leads where id=$1', [novo.id])
    await db.query('select public.excluir_perfil($1)', [ids[1]])
    assert.equal((await rows('select * from perfis where id=$1', [ids[1]])).length, 0)
    await assert.rejects(db.query('select public.excluir_perfil($1)', [ids[1]]))
  })
  await db.close()
})
