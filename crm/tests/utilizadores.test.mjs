...
test('Criação direta exige troca da senha e não envia convite', async () => {
  ...
  assert.equal(app.calls[2][0], 'insert')
  assert.equal(app.calls[2][1].acao, 'criado')
  assert.equal(app.calls[2][1].realizado_por, 'admin-id')
  assert.equal(app.calls[2][1].alvo_email, body.email)
...
test('Com BREVO_API_KEY configurada, envia o email de boas-vindas com a senha e avisa no retorno', async () => {
  ...
  assert.equal(app.calls[2][0], 'insert')
  assert.equal(app.calls[2][1].acao, 'criado')
  assert.equal(app.calls[3][0], 'fetch')
  assert.equal(app.calls[3][1], 'https://api.brevo.com/v3/smtp/email')
  assert.equal(app.calls[3][2].headers['api-key'], 'brevo-test-key')
  const payload = JSON.parse(app.calls[3][2].body)
...
test('Reposição de senha não exige nome/perfil, marca troca obrigatória e não devolve a senha', async () => {
  ...
  assert.equal(app.calls[2][0], 'insert')
  assert.equal(app.calls[2][1].acao, 'senha_reposta')
  assert.equal(app.calls[2][1].alvo_id, alvo)
  assert.equal(app.calls[2][1].realizado_por, 'admin-id')
